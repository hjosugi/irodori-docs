...
How users get Irodori Table and how it updates. Current version: **0.6.0**.
...
So the download channel is live today: push a release tag such as `v0.6.0`.
...
| `cargo install --git` | Rust devs (headless `irodori-server`/CLI) | ✅ unblocked | `irodori-sql` is now a tagged dep (`v0.3.0`); no dev `[patch]` remains |
...
and the
[`packaging/package-managers/`](https://github.com/hjosugi/irodori-table/tree/main/packaging/package-managers)
templates.
...
(How it got here: `irodori-sql` was bumped to `0.3.0`, tagged `v0.3.0`, and
pushed; the workspace dep was repointed from `rev = …` to `tag = "v0.3.0"`.)
...
3. ~~**Unblock cargo**~~ — ✅ done (`irodori-sql v0.3.0` tagged + workspace
